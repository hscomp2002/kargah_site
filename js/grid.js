...
function jshowGrid(indata,gname,targetFile)
{
	...
	this.getGrid = function(gname)
	{
		...
		if(gArgs[gname]['canDelete'] || parseInt(gArgs[gname]['pageCount']) > 1 || gArgs[gname]['canAdd'])
		{
			...
			if(gArgs[gname]['canDelete'])
			{
				out += "<button class='btn btn-info' id='deleteRow-"+gname+"'>\n";
				out += "حذف";
				out += "</button>\n";
			}
			else
			...
			{
				out += "<button class='btn btn-info' id='nextPage-"+gname+"'>\n";
        		        out += "بعدی";
                		out += "</button>\n";
				...
				out += "</select>\n";
				out += "<button class='btn btn-info' id='prePage-"+gname+"'>\n";
		                out += "قبلی";
        		        out += "</button>\n";
			}	
			else	
			...
			{
				out += "<button class='btn btn-info' id='addRow-"+gname+"'>\n";
        		        out += "ثبت";
                		out += "</button>\n";
			}
			else
	...
	function sendRequest(command,field,id,value,ajaxTarget,fromObj,gname)
	{
		...
		$("#"+fromObj).after("<img class=\""+gname+"-status\" src=\"../media/system/images/mootree_loader.gif\" alt=\"Loading . . .\"/>");
	...
	function deleteRequest(ids,ajaxTarget,rowNums,gname)
	{
		...
		$("#deleteRow-"+gname).after("<img class=\""+gname+"-status\" src=\"../media/system/images/mootree_loader.gif\" alt=\"Loading . . .\"/>");
	...
	this.createAddBox = function (inp,gname,columnCount)
	{
		...
		out +='</tr>';
		out +='<tr><td align="left" colspan="'+String(columnCount*2)+'" ><button class="btn btn-info" id="save-'+gname+'" >ذخیره</button><button class="btn btn-info" id="cancelSave-'+gname+'" >انصراف</button></td></tr>';
		out +='</table></div>';
	...
	this.saveData = function (gname,ajax_target,contentDiv,addColCount)
	{
		...
		$("#addRow-"+gname).after("<img class=\""+gname+"-status\" src=\"../media/system/images/mootree_loader.gif\" alt=\"Loading . . .\"/>");
	...
	this.createSearchBox = function ()
	{
		...
		out += '<td><button class="btn btn-info" onclick="grid[\''+gname+'\'].searchGrid(\'grid_search_'+gname+'\');">جستجو</button>';
		out += '</td></tr>';
		if(!hasSer)
			out = '';
		return(out);
	...
	this.init = function(args)
	{
		...
		$("#"+contentDiv).html("<img src='../media/system/images/mootree_loader.gif' />");
